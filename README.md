| Last update: 2026-09-19 17:16 CEST | 1 051 930 malicious IP |
| ---------------------------------- | -------------------- |

🇫🇷 Agrégation de listes d'adresses IP malveillantes de type **scanners** et **bruteforce**, donc à bloquer **UNIQUEMENT** en **entrée** : dans le sens **WAN > LAN/DMZ**

🇬🇧 Aggregation of lists of malicious IP addresses such as scanners and bruteforce, therefore **ONLY** to be blocked in the **WAN > LAN/DMZ** direction

<details>
  <summary><b>🇫🇷⚠️ Introduction : à lire ABSOLUMENT avant utilisation</b></summary>
  <p>

- Agrégation de listes d'adresses IP malveillantes de type **scanners** et **bruteforce**, donc à bloquer **UNIQUEMENT** en **entrée** : dans le sens **WAN > LAN/DMZ** (pour les IP malveillantes en sortie **LAN > WAN**, c'est mon autre repo [malicious-outgoing-ip](https://github.com/romainmarcoux/malicious-outgoing-ip))
...
- Aggregation of lists of malicious IP addresses such as scanners and bruteforce, therefore **ONLY** to be blocked in the **WAN > LAN/DMZ** direction (for malicious outgoing IP **LAN > WAN**, that's my other repo [malicious-outgoing-ip](https://github.com/romainmarcoux/malicious-outgoing-ip))
...
Update of the following table: 2026-09-19 17:16 CEST

| Malicious IP addresses in full-\*                          | %       | Number of IPs |
| ---------------------------------------------------------- | ------- | ------------- |
| Present in 6 sources and more | 6.61 % | 69 541 |
| Present in 5 sources | 2.57 % | 27 036 |
| Present in 4 sources | 2.94 % | 30 932 |
| Present in 3 sources | 5.75 % | 60 557 |
| Present in 2 sources | 13.54 % | 142 491 |
| Present in 1 source | 68.57 % | 721 373 |
| Total | 100 % | 1 051 930 |

Update of the common IP table with the FortiGate ISDB Malicious-Malicious.Server: 2026-09-19 01:50 CEST
...
<details>
  <summary><b>Ready-to-use FortiGate configuration</b></summary>
  <p>
Creating threat feeds for FortiOS 7.4.4+:

```
config system external-resource
    edit "malicious-ip-sekio-300k-aa"
        set type address
        set resource "https://raw.githubusercontent.com/romainmarcoux/malicious-ip/main/full-300k-aa.txt"
        set refresh-rate 60
    next
    edit "malicious-ip-sekio-300k-ab"
        set type address
        set resource "https://raw.githubusercontent.com/romainmarcoux/malicious-ip/main/full-300k-ab.txt"
        set refresh-rate 60
    next
    edit "malicious-ip-sekio-300k-ac"
        set type address
        set resource "https://raw.githubusercontent.com/romainmarcoux/malicious-ip/main/full-300k-ac.txt"
        set refresh-rate 60
    next
    edit "malicious-ip-sekio-300k-ad"
        set type address
        set resource "https://raw.githubusercontent.com/romainmarcoux/malicious-ip/main/full-300k-ad.txt"
        set refresh-rate 60
    next
end
```
